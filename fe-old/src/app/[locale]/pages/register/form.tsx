...
export default function Registerform() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!username.trim()) newErrors.username = "Vui lòng nhập họ tên";
    ...
    const result = await register(username, password, email, phone);
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex items-center justify-center px-5">
        <form className="relative my-20 flex flex-col gap-5 p-5 w-[450px] rounded-2xl font-sans shadow-lg backdrop-blur-md bg-white/10 border border-white/20 overflow-visible">
          <h2 className="text-2xl font-bold text-center text-white mb-2">
            Đăng ký
          </h2>

          <div className="flex flex-col gap-2">
            <label className="text-white font-semibold">Họ tên</label>
          ...
          <div className="flex flex-col gap-2">
            <label className="text-white font-semibold">Số điện thoại</label>
            <div className="flex items-center border border-[#ecedec] rounded-xl h-12 pl-2 focus-within:border-[#2d79f3] transition">
              <input
                type="tel"
