      ...
      <div className="relative">
        <AnimatedList className="w-full mt-8">
          <BillingItem
            key="item-1"
            icon={<ExchangeIcon />}
            text="Hệ thống tự động cộng Xu và kiểm tra mốc nạp"
            latency="0.5 s"
          />
          <BillingItem
            key="item-2"
            icon={<LocationIcon />}
            text="Xác thực giao dịch thành công qua Ngân hàng"
            latency="1.2 s"
          />
          <BillingItem
            key="item-3"
            icon={<OptionsIcon />}
            text="Admin vừa kích hoạt sự kiện khuyến mãi 100%"
            latency="8 ms"
          />
          <BillingItem
            key="item-4"
            icon={<BillingIcon />}
            text="Vietcod đã gửi thông báo xác nhận đơn hàng"
            latency="0.9 s"
          />
          <BillingItem
            key="item-5"
            icon={<PaymentsIcon />}
            text="Đối tác đã thực hiện rút doanh thu về ví"
            latency="1.5 s"
          />
        </AnimatedList>
      </div>
...
export function BillingItem({
  ...
  return (
    <div
      className={`flex relative rounded-xl border-[.75px] w-[440px] usage-item-gradient border-white/20 items-center py-[12px] px-[16px] ${className}`}
    >
  ...
  return (
    <div className="flex flex-col text-white absolute left-[20px] sm:left-[40px] xl:left-[40px] bottom-[40px] max-w-[3300px]">
      ...
      <p className="mt-4 text-white/60 leading-6 max-w-[350px]">
        Hệ thống tự động hóa hoàn toàn quy trình thanh toán và phát quà, giúp
        bạn vận hành kinh doanh rảnh tay.
      </p>
    </div>
  );
}
